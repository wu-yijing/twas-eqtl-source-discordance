...
- `03_redraw_Fig6.py` — Fig. 6 第二版，**已被 `08` 取代**；默认 `SystemExit`，如确需运行设 `FIG6_FROM_03=1`。
- `02_redraw_Fig3.py` 内的 Fig6 段 — 旧版（ylim = 24、图例在内），默认跳过；如确需设 `FIG6_FROM_02=1`。**不要去掉这个护栏。**
- `_recon_Fig6.py` — 一次性对账脚本（`_` 前缀，不随包发布）。
